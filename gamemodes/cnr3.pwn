...
#include <foreach>
#include <aka>
#include <EVF> //Extended vehicle functions
...
#define DIALOG_CP 16
#define DIALOG_EVENT 17
#define DIALOG_BAN 18
#define DIALOG_SUSPEND 19
//--- C L A S S   I D S-----//
#define TEAM_FBI 0
#define TEAM_CIA 1
#define TEAM_ARMY 2
...
new Text3D:aodlabel;
//------ M Y S Q L  R E L A T E D------//
new dbHandle;

//---- S T O R E  R E L A T E D----//
new StoreInteriorCP[MAX_STORES];
new StoreExteriorCP[MAX_STORES];
//------ T E A M   R E L A T E D ------//
...
//------- S P A W N   R E L A T E D--------//
new IsPlayerSpawned[MAX_PLAYERS];
//------- A N T I   S P A M -------------//
new AntiSpam[MAX_PLAYERS];
//------ C L A S S   S E L E C T I O N   B O S-------//
...
//HEX COLORS FOR TEAMS

//----- S T O R E    C H E C K P O I N T S ------------//
...
//---------- A T M   C H E C K P O I N T S   A N D   O B J E C T S   P O S I T I O N------------//
...
//----------- C L A S S    S E L E C T I O N    T E X T D R A W S-----------------------//
...
new hInfo[MAX_PLAYER_NAME][e_HouseINFO];
new houseid;
//---------- S U P A   I T E M S ---------//
...
//----------- G A M E M O D E   C A L L B A C K S ---------------//
...
public OnGameModeInit()
{
	...
	{
	    StoreInteriorCP[x] = CreateDynamicCP(storeInteriorCoordinates[x][InteriorX], storeInteriorCoordinates[x][InteriorY], storeInteriorCoordinates[x][InteriorZ], MAX_STORE_CP_SIZE, .interiorid = storeInteriorCoordinates[x][InteriorID], .worldid = storeInteriorCoordinates[x][worldid]);
	...
	new HEX;
 	cache_get_field_content_float(0, "HouseEX", HEX);
	new HEY;
	cache_get_field_content_float(0, "HouseEY", HEY);
	new HEZ;
	cache_get_field_content_float(0, "HouseEZ", HEZ);
	new HID;
	cache_get_field_content_int(0, "HouseID", HID);
	CreateDynamicCP(HEX, HEY, HEZ, 1.5, .worldid = 0, .interiorid = 0, .streamdistance = 20.0);
...
public OnPlayerEnterDynamicCP(playerid, checkpointid)
{
    ...
    AntiSpam[playerid] = gettime() + 3; // cooldown, so that a player does not get teleported in and out..
  //  return 1;
}
...
public OnPlayerConnect(playerid) // So player just connected ,you want to check if he is regis
{
	...
	SendClientMessage(playerid, -1, "{FF0000}[WARNING] {FFFFFF}This server contains explicit content, play at your own risk.");
 	...
 	//ban and suspend checking
 	cache_get_field_content_int(0, "banned", pInfo[playerid][pBanned]);
 	if(pInfo[playerid][pBanned] == 1)
 	{
 	new bannedby = cache_get_field_content_int(0, "bannedby");
 	new banreason = cache_get_field_content_int(0, "banreason");
 	new bandialog[300];
 	format(bandialog, 300, "{FFFFFF}In-game name: %s\nBanned By: %s\n Ban Reason: %s\n\n{C9C9C9}If you feel you've been wrongfully banned, you can appeal at "WEBSITE_NAME". You can also apologize if you hacked, we might be kind enough to unban you.", name, bannedby, banreason);
 	ShowPlayerDialog(playerid, DIALOG_BAN, DIALOG_STYLE_MSGBOX, "BANNED", bandialog, "OK", "EXIT");
 	}
 	
 	else if(stime > gettime())
 	{
 	if(pInfo[playerid][pBanned] == 2)
 	{
 	
 	new bannedby = cache_get_field_content_int(0, "bannedby");
 	new banreason = cache_get_field_content_int(0, "banreason");
 	new bandialog[300];
 	format(bandialog, 300, "{FFFFFF}In-game name: %s\nBanned By: %s\n Ban Reason: %s\nTime left:%i hour(s)\n\n{C9C9C9}If you feel you've been wrongfully banned, you can appeal at "WEBSITE_NAME". You can also apologize if you hacked, we might be kind enough to unban you.", name, bannedby, banreason, stime-gettime()/3600);
 	ShowPlayerDialog(playerid, DIALOG_BAN, DIALOG_STYLE_MSGBOX, "BANNED", bandialog, "OK", "EXIT");
 	}
    }
 	else
...
forward UpdateColor(playerid);
public UpdateColor(playerid)
{
	if(pInfo[playerid][pWanted] == 6)
	return SetPlayerColor(playerid, 0xFFFFFFAA);

    if(pInfo[playerid][pWanted] == 7||8||9||10||11)
    return SetPlayerColor(playerid, 0xFF4500AA);
    
	if(pInfo[playerid][pWanted] >= 12)
	return SetPlayerColor(playerid, 0x8B0000FF);
			...
			if(response)
				{
				 ...
				 else
				 {
				    ...
				    format(query, 320, "INSERT INTO `playerdata` (`username`, `password`, `pjobselected`, `wanted`) VALUES ('%s', '%s', '0', '0')", lName, hash);
...
//Ropes, Scissors, Bobby Pins, Metal Melters, MoneyBag
    ...
    if(dialogid == DIALOG_CP)
		{
		if(!response)
		return 0;
		if(listitem == 0)
		{
		if(pInfo[playerid][Connectionlog] == 0)
		{
		new controlp[156];
		pInfo[playerid][Connectionlog] = 1;
		format(controlp, 156, "Connection Log\t%d\nEvent Teleports\t%d\nHitmarker\t\t%d\nAuto Save\t\t%d", pInfo[playerid][Connectionlog], pInfo[playerid][eventtp], pInfo[playerid][hitmarker], pInfo[playerid][AutoSave]);
		ShowPlayerDialog(playerid, DIALOG_CP, DIALOG_STYLE_LIST,"CONTROL PANEL || 1 = ENABLED || 0 = DISABLED", controlp, "SELECT", "CANCEL");
		new query[128], ttname[24];
		GetPlayerName(playerid, ttname, 24);
		format(query, 128, "UPDATE `playerdata` SET `connectionlog` = '0' WHERE `username` = '%s'", ttname);
		mysql_tquery(dbHandle, query);
		}
		else if(pInfo[playerid][Connectionlog] == 1)
		{
		new controlp[156];
		pInfo[playerid][Connectionlog] = 0;
		format(controlp, 156, "Connection Log\t%d\nEvent Teleports\t%d\nHitmarker\t\t%d\nAuto Save\t\t%d", pInfo[playerid][Connectionlog], pInfo[playerid][eventtp], pInfo[playerid][hitmarker], pInfo[playerid][AutoSave]);
		ShowPlayerDialog(playerid, DIALOG_CP, DIALOG_STYLE_LIST,"CONTROL PANEL || 1 = ENABLED || 0 = DISABLED", controlp, "SELECT", "CANCEL");
		new query[128], ttname[24];
		GetPlayerName(playerid, ttname, 24);
		format(query, 128, "UPDATE `playerdata` SET `connectionlog` = '1' WHERE `username` = '%s'", ttname);
		mysql_tquery(dbHandle, query);
		}
		}
		...
		if(listitem == 1)
		{
		if(pInfo[playerid][eventtp] == 0)
		{
		pInfo[playerid][eventtp] = 1;
		new controlp[156];
		format(controlp, 156, "Connection Log\t%d\nEvent Teleports\t%d\nHitmarker\t\t%d\nAuto Save\t\t%d", pInfo[playerid][Connectionlog], pInfo[playerid][eventtp], pInfo[playerid][hitmarker], pInfo[playerid][AutoSave]);
		ShowPlayerDialog(playerid, DIALOG_CP, DIALOG_STYLE_LIST,"CONTROL PANEL || 1 = ENABLED || 0 = DISABLED", controlp, "SELECT", "CANCEL");
		new query[128], ttname[24];
		GetPlayerName(playerid, ttname, 24);
		format(query, 128, "UPDATE `playerdata` SET `eventtp` = '1' WHERE `username` = '%s'", ttname);
		mysql_tquery(dbHandle, query);
		}
		else if(pInfo[playerid][eventtp] == 1)
		{
		pInfo[playerid][eventtp] = 0;
		new controlp[156];
		format(controlp, 156, "Connection Log\t%d\nEvent Teleports\t%d\nHitmarker\t\t%d\nAuto Save\t\t%d", pInfo[playerid][Connectionlog], pInfo[playerid][eventtp], pInfo[playerid][hitmarker], pInfo[playerid][AutoSave]);
		ShowPlayerDialog(playerid, DIALOG_CP, DIALOG_STYLE_LIST,"CONTROL PANEL || 1 = ENABLED || 0 = DISABLED", controlp, "SELECT", "CANCEL");
		new query[128], ttname[24];
		GetPlayerName(playerid, ttname, 24);
		format(query, 128, "UPDATE `playerdata` SET `eventtp` = '0' WHERE `username` = '%s'", ttname);
		mysql_tquery(dbHandle, query);
		}
		}
		if(listitem == 2)
		{
		if(pInfo[playerid][hitmarker] == 0)
		{
		pInfo[playerid][hitmarker] = 1;
		new controlp[156];
		format(controlp, 156, "Connection Log\t%d\nEvent Teleports\t%d\nHitmarker\t\t%d\nAuto Save\t\t%d", pInfo[playerid][Connectionlog], pInfo[playerid][eventtp], pInfo[playerid][hitmarker], pInfo[playerid][AutoSave]);
		ShowPlayerDialog(playerid, DIALOG_CP, DIALOG_STYLE_LIST,"CONTROL PANEL || 1 = ENABLED || 0 = DISABLED", controlp, "SELECT", "CANCEL");
		new query[128], ttname[24];
		GetPlayerName(playerid, ttname, 24);
		format(query, 128, "UPDATE `playerdata` SET `hitmarker` = '0' WHERE `username` = '%s'", ttname);
		mysql_tquery(dbHandle, query);
		}
		else if(pInfo[playerid][hitmarker] == 1)
		{
		pInfo[playerid][hitmarker] = 0;
		new controlp[156];
		format(controlp, 156, "Connection Log\t%d\nEvent Teleports\t%d\nHitmarker\t\t%d\nAuto Save\t\t%d", pInfo[playerid][Connectionlog], pInfo[playerid][eventtp], pInfo[playerid][hitmarker], pInfo[playerid][AutoSave]);
		ShowPlayerDialog(playerid, DIALOG_CP, DIALOG_STYLE_LIST,"CONTROL PANEL || 1 = ENABLED || 0 = DISABLED", controlp, "SELECT", "CANCEL");
		new query[128], ttname[24];
		...
		if(listitem == 3)
		{
		if(pInfo[playerid][AutoSave] == 0)
		{
		pInfo[playerid][AutoSave] = 1;
		new controlp[156];
		format(controlp, 156, "Connection Log\t%d\nEvent Teleports\t%d\nHitmarker\t\t%d\nAuto Save\t\t%d", pInfo[playerid][Connectionlog], pInfo[playerid][eventtp], pInfo[playerid][hitmarker], pInfo[playerid][AutoSave]);
		ShowPlayerDialog(playerid, DIALOG_CP, DIALOG_STYLE_LIST,"CONTROL PANEL || 1 = ENABLED || 0 = DISABLED", controlp, "SELECT", "CANCEL");
		new query[128], ttname[24];
		GetPlayerName(playerid, ttname, 24);
		format(query, 128, "UPDATE `playerdata` SET `autosave` = '1' WHERE `username` = '%s'", ttname);
		mysql_tquery(dbHandle, query);
		}
		else if(pInfo[playerid][AutoSave] == 1)
		{
		pInfo[playerid][AutoSave] = 0;
		new controlp[156];
		format(controlp, 156, "Connection Log\t%d\nEvent Teleports\t%d\nHitmarker\t\t%d\nAuto Save\t\t%d", pInfo[playerid][Connectionlog], pInfo[playerid][eventtp], pInfo[playerid][hitmarker], pInfo[playerid][AutoSave]);
		ShowPlayerDialog(playerid, DIALOG_CP, DIALOG_STYLE_LIST,"CONTROL PANEL || 1 = ENABLED || 0 = DISABLED", controlp, "SELECT", "CANCEL");
		new query[128], ttname[24];
	...
	if(dialogid == DIALOG_EVENT)
		...
		SetPlayerPos(playerid, -828.4146,504.3322,1358.4519);
		SetPlayerVirtualWorld(playerid, EVENTVIRTUALWORLD);
		SetPlayerInterior(playerid, 1);
		}
		if(listitem == 1)
		{
		SetPlayerPos(playerid, 1452.9921,-1068.4650,213.3828);
		SetPlayerVirtualWorld(playerid, EVENTVIRTUALWORLD);
		SetPlayerInterior(playerid, 0);
		}
		if(listitem == 2)
		{
		SetPlayerPos(playerid, 1392.0542,-17.3458,1000.9190);
		SetPlayerVirtualWorld(playerid, EVENTVIRTUALWORLD);
		SetPlayerInterior(playerid, 1);
		}
		if(listitem == 3)
        ...
        if(listitem == 4)
		{
		SetPlayerPos(playerid, -1397.5299,942.2256,1033.2960);
		SetPlayerVirtualWorld(playerid, EVENTVIRTUALWORLD);
		SetPlayerInterior(playerid, 15);
		}
		if(listitem == 5)
		{
		SetPlayerPos(playerid, -1456.6068,1603.2526,1052.7500);
		SetPlayerVirtualWorld(playerid, EVENTVIRTUALWORLD);
...
public OnPlayerTryLogin(playerid)
{
	...
	{
	    new lquery[256]; // Can you not fuck around when i am doing stuff? hard for me :#
		...
		new lName[24];
		GetPlayerName(playerid, lName, 24);
		format(lquery, 256, "{FFFFFF}Welcome to San Fierro Cops and Robbers. The account {00FF00}(%s){FFFFFF} is registered.\t\n\n\nPlease log-in by entering your password.\t\n\n\n{696969}If this account does not belong to you, please leave and join with a new nickname.", lName);
...
public OnPlayerDisconnect(playerid, reason)
{
 	...
 	if(pInfo[playerid][pLoggedIn] == 1)
	{
	    ...
	    new query[256];
	    format(query, 256, "UPDATE `playerdata` SET `score` = '%d', `kills` = '%d', `deaths` = '%d', `admin` = '%d', `pXP` = '%d' `VIP` = '%d', `money` = '%d', `pjobselected` = '1', `pjob` ='%d', `wanted` = '%d' WHERE `id` = '%d'", score, pInfo[playerid][pKills], pInfo[playerid][pDeaths], pInfo[playerid][pAdmin], pInfo[playerid][pXP], pInfo[playerid][pVIP], money, pInfo[playerid][pJob], pInfo[playerid][pWanted], pInfo[playerid][pDBID]);
...
//-------------------- A N T I   C H E A T   C A L L B A C K S -----------------------------

new Test_String[80], hname[24];

public OnPlayerFakeKill(playerid, spoofedid, spoofedreason, faketype)
{
    GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} %s(%d) is suspected of fake killing %d.{00FF00} [TYPE: %d] [REASON: %d]", playerid, spoofedid, faketype, spoofedreason);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of fake connecting!", playerid);
   	SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of using jetpack cheats!", playerid);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of spamming the chat!", playerid);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of using weapon cheats weapon %d ammo %d type %d!", playerid, weaponid, ammo, hacktype);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of speed cheats type !", playerid, speedtype);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of using airbreak/teleport cheats type %d !", playerid, breaktype);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of car spamming %d vehicles !", playerid, number);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
public OnPlayerCarTroll(playerid, vehicleid, trolledid, trolltype)
{
    ...
    if(trolledid == INVALID_PLAYER_ID)
        format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of car troll cheats vehicle %d type %d !", playerid, vehicleid, trolltype);
    else
        format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of car troll cheats on ID %d vehicle %d type %d !", playerid, trolledid, vehicleid, trolltype);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of using money cheats for %d !", playerid, amount);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of using car swing cheats vehicle %d !", playerid, vehicleid);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of car particle spam cheats vehicle %d !", playerid, vehicleid);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
public OnVehicleModEx(playerid, vehicleid, componentid, illegal)
{
    ...
    {
    	GetPlayerName(playerid, hname, 24);
        format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is suspected of car mod cheats component %d vehicle %d !", playerid, componentid, vehicleid);
        SCMToAdmins(-1, Test_String);
        print(Test_String);
    }
    return 1;
...
{
	GetPlayerName(playerid, hname, 24);
    format(Test_String, sizeof(Test_String), "{1E90FF}[ADMIN]{FFFFFF} (%s)%d is lagging type %d ping %d !", playerid, lagtype, ping);
    SCMToAdmins(-1, Test_String);
    print(Test_String);
    return 1;
}
...
CMD:dndall(playerid, params[])
{
	...
	{
	dndid[i] = 1;
	format(msg, sizeof(msg), "{FFD700}[DO NOT DISTURB]{FFFFFF} You've blocked everyone from sending you private messages.", dname);
	SendClientMessage(playerid, -1, msg);
	}
	return 1;
}
...
{   new action[65];
	...
	format(string, 128, "*** %s(%d) %s.", mName, playerid, action);
...
public OnPlayerText(playerid, text[])
{
	...
	{
		new string[89]; // creates a new variable
		format(string, sizeof(string), "{FF0000}[ERROR]{FFFFFF} You cannot chat for %d seconds in the main chat as you are muted.", unmutetime - gettime()); // sets the string
...
CMD:aod(playerid, params[])
{
	...
	{
	AdminUsage(playerid, "You've disabled administration mode.");
	aod[playerid] = false;
	SetPlayerHealth(playerid, 100);
	DestroyDynamic3DTextLabel(aodlabel);
	...
	GetPlayerPos(playerid, PosX, PosY, PosZ);
	AdminUsage(playerid, "You've enabled administration mode.");
	aodlabel = CreateDynamic3DTextLabel("Admin on Duty", 0x1E90FFFF, PosX, PosY, PosZ, 100.0, 0, 1);
	Attach3DTextLabelToPlayer(aodlabel, playerid, 0.0, 0.0, 0.3);
...
CMD:warn(playerid, params[])
{
	...
	SetTimer("kickmessage", 250, false);
...
forward kickmessage(tid);
public kickmessage(tid)
{
	Kick(tid);
	return 1;
}
...
CMD:mute(playerid, params[])
{
	...
	Mute[targetid] = 1;
	format(MuteSent, 128, "{1E90FF}[ADMIN] {FFFFFF}%s(%d) has been muted by %s(%d) for %d seconds. {00FF00}[REASON: %s]", targetname, targetid, mName, playerid, time, reason);
	SendClientMessageToAll(-1, MuteSent);
	unmutetime = gettime() + time;
...
{
	new tid, tname[24], msg[96], tIP[16];
	if(pInfo[playerid][pAdmin] >= 2)
	{
	if(sscanf(params, "u", tid))
...
{
	new tname[24], tid, aname[24], hours, days, reason[48], query[176], msgall[128];
	GetPlayerName(playerid, aname, 24);
	GetPlayerName(tid, tname, 24);
	if(pInfo[playerid][pAdmin] >= 2)
	{
	if(sscanf(params, "uiis", tid, hours, days, reason))
	return AdminUsage(playerid, "/suspend [PLAYERID] [HOURS] [DAYS] [REASON]");
	
	stime = gettime() + hours*3600 + days*3600*24;
	format(query, 128, "UPDATE `playerdata` SET `ban` = '2', `bannedby` = '%s', `banreason` = '%s', WHERE `username` = '%s'", aname, reason, tname);
	mysql_tquery(dbHandle, query);
	format(msgall, 128, "{1E90FF}[ADMIN]{FFFFFF} %s has suspended %s(%d) for %i hours and %i days. {00FF00}[REASON: %s]", aname, tname, tid, hours, days, reason);
	SendClientMessageToAll(-1, msgall);
	SetTimer("kickmessage", 250, false);
	}
	else
...
CMD:freeze(playerid, params[])
{
	...
	{
	if(sscanf(params, "u", tid))
	return AdminUsage(playerid, "/freeze [PLAYERID]");
...
CMD:setvw(playerid, params[])
{
	...
	SetPlayerVirtualWorld(tid, vworldid);
	SendClientMessage(playerid, -1, pmsg);
	SendClientMessage(tid, -1, tmsg);
	AdminLogs(logs);
	}
...
{
	new targetid, reason[48], tname[24], aname[24], string[146], query[128];
	GetPlayerName(playerid, aname, 24);
	GetPlayerName(targetid, tname, 24);
	...
	return AdminUsage(playerid, "/ban [PLAYERID] [REASON]");
	
	format(string, 146, "{1E90FF}[ADMIN]{FFFFFF} %s has banned %s(%d). {00FF00}[REASON: %s]", aname, tname, targetid, reason);
	format(query, 128, "UPDATE `playerdata` SET `ban` = '1', `bannedby` = '%s', `banreason` = '%s' WHERE `username` = '%s'", aname, reason, tname);
	SendClientMessageToAll(-1, string);
	SetTimer("kickmessage", 250, false);
	}
	else
...
CMD:event(playerid, params[])
{
	...
	if(isnull(params)) // if they didn't type something after /create
	{
	    ...
	    return AdminUsage(playerid, "/event [ARENAS/BRING]");
	}
	
	if(!strcmp(option, "arenas", true))
	{
	    ShowPlayerDialog(playerid, DIALOG_EVENT, DIALOG_STYLE_LIST, "EVENT ARENAS", "Liberty City Inside\nATP(LS ROOF)\nWarehouse 1\nWarehouse 2\nDerby 1\nDerby 2", "GO", "EXIT");
		return 1;
	}
...
{
	new tname[24], string[146], query[128];

	if(pInfo[playerid][pAdmin] >= 5)
	{
	if(sscanf(params, "u", tname))
	return AdminUsage(playerid, "/unban [PLAYERNAME]");

	format(string, 146, "{1E90FF}[ADMIN]{FFFFFF} %s has been un-banned from the server.", tname);
	format(query, 128, "UPDATE `playerdata` SET `ban` = '0', `bannedby` = '', `banreason` = '' WHERE `username` = '%s'", tname);
	AdminLogs(string);
	SendClientMessageToAll(-1, string);
	}
	else
...
CMD:check(playerid,params[])
{
    ...
    if(pInfo[playerid][pAdmin] >= 5)
    {
    if(sscanf(params,"u",ppID)) return AdminUsage(playerid, "/check [PLAYERID]");
...
{
	if(sscanf(params, "d", hInfo[houseid][HousePrice]))
	return AdminUsage(playerid, "/createhouse [PRICE]");
...
createhouse(playerid)
{
	...
	hInfo[houseid][HouseOwned] = 0;
	new label[145];
	GetPlayerPos(playerid, hInfo[houseid][HouseEX], hInfo[houseid][HouseEY], hInfo[houseid][HouseEZ]);
	
	hInfo[houseid][HouseCP] = CreateDynamicCP(hInfo[houseid][HouseEX], hInfo[houseid][HouseEY], hInfo[houseid][HouseEZ], 1.5, .interiorid = 0, .worldid = 0, .streamdistance = 55);
	format(label, 145, "{FFD700}House Title: {FFFFFF}N/A{FFD700}\nHouse Owner: {FFFFFF}No-one{FFD700}\nHouse Price: {FFFFFF}%d", hInfo[houseid][HousePrice]);
	hInfo[houseid][HouseDetails] = CreateDynamic3DTextLabel(label, 0xF3F3F3, hInfo[houseid][HouseEX], hInfo[houseid][HouseEY], hInfo[houseid][HouseEZ], 25, .worldid = 0, .interiorid = 0, .streamdistance = 55);
	
	new query[300];
	format(query, 300, "INSERT INTO `houses`(`HouseEX`, `HouseEY`, `HouseEZ`, `HousePrice`, `HouseOwned`, `HouseTitle`) VALUES ('%f', '%f', '%f', '%d', '0', 'N/A')", hInfo[houseid][HouseEX], hInfo[houseid][HouseEY], hInfo[houseid][HouseEZ], hInfo[houseid][HousePrice]);
	mysql_tquery(dbHandle, query);
...
{
	new queryzz[60];
	format(queryzz, 60, "SELECT * FROM `houses`");
	mysql_tquery(dbHandle, queryzz, "OnServerLoadHouses");
}

forward OnServerLoadHouses();
public OnServerLoadHouses()
{   new label[145];
	...
	for(new i = 0; i < rows; i++)
	{
	    ...
	    {
			cache_get_field_content_int(i, "HouseOwned", hInfo[z][HouseOwned]);
  			if(hInfo[z][HouseOwned] == 0)
		    {
		        ...
		        hInfo[z][HouseCP] = CreateDynamicCP(hInfo[z][HouseEX], hInfo[z][HouseEY], hInfo[z][HouseEZ], 1.5, .interiorid = 0, .worldid = 0, .streamdistance = 55);
		        format(label, 145, "{FFD700}House Title: {FFFFFF}N/A{FFD700}\nHouse Owner: {FFFFFF}No-one{FFD700}\nHouse Price: {FFFFFF}%d", hInfo[z][HousePrice]);
		        UpdateDynamic3DTextLabelText(hInfo[z][HouseDetails], 0xFFFFFF, label);
			}
			else
			if(hInfo[z][HouseOwned] == 1)
			{
				hInfo[z][HouseEX] = cache_get_field_content_float(i, "HouseEX");
		        hInfo[z][HouseEY] = cache_get_field_content_float(i, "HouseEY");
		        hInfo[z][HouseEZ] = cache_get_field_content_float(i, "HouseEZ");
		        ...
		        hInfo[z][HousePrice] = cache_get_field_content_int(i, "HousePrice");
		        hInfo[z][HouseCP] = CreateDynamicCP(hInfo[z][HouseEX], hInfo[z][HouseEY], hInfo[z][HouseEZ], 1.5, .interiorid = 0, .worldid = 0, .streamdistance = 55);
		        format(label, 145, "{FFD700}House Title: {FFFFFF}%s{FFD700}\nHouse Owner: {FFFFFF}%e{FFD700}\nHouse Price: {FFFFFF}%d", hInfo[z][HouseTitle], hInfo[z][HousePrice], hInfo[z][HouseOwner]);
		        UpdateDynamic3DTextLabelText(hInfo[z][HouseDetails], 0xFFFFFF, label);
...
CMD:buyhouse(playerid, params[])
{
	...
	cache_get_field_content_int(hid, "HouseID", hInfo[houseid][HouseID]);
	{
		...
		mysql_format(dbHandle, queryq, sizeof(queryq), "UPDATE `houses` SET `HouseOwned` = '1', `HouseOwner` = '%e' WHERE `HouseID` = '%d' ", pqname, hid);
		mysql_tquery(dbHandle, queryq);
	}
	return 1;
}

//ye ajeeb sa Y ban rhaa h HOUSE OWNER pe..
