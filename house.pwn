...
#define     LOCK_MODE_KEYS  	(2)
#define     LOCK_MODE_OWNER     (3)
...
stock LoadHouseKeys(playerid)
{
    ...
    new query[72], name[MAX_PLAYER_NAME];
    GetPlayerName(playerid, name, MAX_PLAYER_NAME);
    mysql_format(SQLHandle, query, sizeof(query), "SELECT * FROM housekeys WHERE Player='%e'", name);
 	...
 	if(rows)
  	{
		...
		while(loaded < rows)
		{
			...
			format(HouseData[id][Address], MAX_HOUSE_ADDRESS, "%d, %s, %s", id, GetZoneName(HouseData[id][houseX], HouseData[id][houseY], HouseData[id][houseZ]), GetCityName(HouseData[id][houseX], HouseData[id][houseY], HouseData[id][houseZ]));
   			owned = strcmp(HouseData[id][Owner], "-");
			...
			HouseData[id][HousePickup] = CreateDynamicPickup((owned) ? 19522 : 1273, 1, HouseData[id][houseX], HouseData[id][houseY], HouseData[id][houseZ]);
			HouseData[id][HouseIcon] = CreateDynamicMapIcon(HouseData[id][houseX], HouseData[id][houseY], HouseData[id][houseZ], (owned) ? 32 : 31, 0);
...
public OnFilterScriptInit()
{
	...
	foreach(new i : Player) LoadHouseKeys(i);
					    ...
					    switch(HouseData[i][LockMode])
					    {
							...
							case LOCK_MODE_KEYS:
							{
							    ...
							    if(!gotkeys)
							    {
									...
									GetPlayerName(playerid, name, MAX_PLAYER_NAME);
									if(!strcmp(HouseData[i][Owner], name)) gotkeys = 1;
								}
...
CMD:myhousekeys(playerid, params[])
{
    new query[200], name[MAX_PLAYER_NAME], Cache: mykeys;
    GetPlayerName(playerid, name, MAX_PLAYER_NAME);
    mysql_format(SQLHandle, query, sizeof(query), "SELECT HouseID, FROM_UNIXTIME(Date, '%%d/%%m/%%Y %%H:%%i') as KeyDate FROM housekeys WHERE Player='%e' ORDER BY Date DESC LIMIT 0, 15", name);
	mykeys = mysql_query(SQLHandle, query);
	ListPage[playerid] = 0;
	...
	if(rows) {
		...
		ShowPlayerDialog(playerid, DIALOG_HOUSE+19, DIALOG_STYLE_TABLIST_HEADERS, "My Keys (Page 1)", list, "Next", "Close");
	}else{
...
{
    if(InHouse[playerid] == INVALID_HOUSE_ID) return SendClientMessage(playerid, 0xE74C3CFF, "You're not in a house.");
    new name[24], id, houseid = InHouse[playerid];
	GetPlayerName(playerid, name, MAX_PLAYER_NAME);
	if(strcmp(HouseData[houseid][Owner], name)) return SendClientMessage(playerid, 0xE74C3CFF, "You're not the owner of this house.");
	...
	new query[128];
	mysql_format(SQLHandle, query, sizeof(query), "INSERT INTO housekeys SET HouseID=%d, Player='%e', Date=UNIX_TIMESTAMP()", houseid, name);
	mysql_tquery(SQLHandle, query, "", "");
	
	format(query, sizeof(query), "You've given keys to %s for this house.", name);
...
CMD:takehousekeys(playerid, params[])
{
	...
	new query[128];
	mysql_format(SQLHandle, query, sizeof(query), "DELETE FROM housekeys WHERE HouseID=%d AND Player='%e'", houseid, name);
	mysql_tquery(SQLHandle, query, "", "");

	format(query, sizeof(query), "You've taken keys from %s for this house.", name);
	SendClientMessage(playerid, -1, query);
...
CMD:kickfromhouse(playerid, params[])
{
	...
	SetPVarInt(id, "HousePickupCooldown", tickcount()+8000);
